...
3. `apply_update_and_restart()` — pisze `.bat` w `%TEMP%` i odpala go odłączonego; dopiero **gdy to się uda** `WebUI._restart_into()` robi `stop()` (domknięcie sesji aparatu — porzucone PTP = BUSY po restarcie) i `os._exit(0)`. Gdy uruchomienie aktualizatora rzuci, aplikacja ZOSTAJE otwarta z błędem w logu — `os._exit(0)` w `finally` sprawiał, że każda wtopa wyglądała identycznie: program znikał, nic się nie zmieniało, nie było śladu.
4. `.bat` czeka ~3 s, robi `robocopy _internal /MIR` (stare DLL-e z poprzedniej wersji znikają) + `robocopy /E` reszty **bez kasowania** (`.env`, `photos/`, ręcznie dołożony `EDSDK.dll` zostają — one żyją obok `.exe`, patrz `PROJECT_DIR`), startuje `.exe` i kasuje sam siebie. Wszystko leci do `%TEMP%\camera_capture_update.log`, a przy błędzie okno zostaje otwarte z `pause`.

Trzy pułapki, na których to płonęło (objaw za każdym razem ten sam — okno migało pół sekundy i wersja zostawała stara):
- **Ścieżka nie-ASCII albo ze spacją.** `.bat` był zapisywany w `encoding="ascii"`, więc profil `C:\Users\Michał` wywalał zapis wyjątkiem, a spacja w profilu psuła komendę przez reguły zdejmowania cudzysłowów w `cmd /c`. Teraz wszystkie ścieżki w skrypcie idą przez `_short()` (`GetShortPathNameW` → `C:\Users\MICHA~1\…`, bez spacji i bez ogonków), a `cmd.exe /c "<bat>"` jest podawane jako JEDEN string, nie lista.
- **Cały skrypt musi być ASCII** — cmd czyta `.bat` w codepage OEM. Żadnych polskich znaków ani myślników typograficznych w `echo`.
- **`.bat` i log leżą w `%TEMP%`, poza kasowanym drzewem** `cc_update_*` — inaczej `rd /s /q` próbowało usunąć katalog z działającym skryptem (i jego `cwd`).

Czekanie na PID przez `tasklist` wyleciało: format wyjścia zależy od języka Windows. Zamiast tego stała pauza + `robocopy /R:20 /W:1` — dopóki aplikacja trzyma pliki, kopiowanie się ponawia zamiast wywalać.
